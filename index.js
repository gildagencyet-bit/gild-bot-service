...
// ── IMAGE URLS ──
const IMG = {
  welcome:       'https://i.postimg.cc/28c5Jg4h/1777350054899.png',
  mainMenu:      'https://i.postimg.cc/zXsf8KcY/1777273692663.png',
  form:          'https://i.postimg.cc/tT4q25Yn/1777273702539.png',
  legacy:        'https://i.postimg.cc/PNsksz5H/1777273330007.png',
  faq:           'https://i.postimg.cc/BQKqYtnG/1777273717527.png',
  platform:      'https://i.postimg.cc/tRVTfBBc/1777273732729.png',
  support:       'https://i.postimg.cc/fy4wDh8B/1777273744115.png',
  packages:      'https://i.postimg.cc/0NbxSN3B/1777274294959.png',
  currency:      'https://i.postimg.cc/hvqqN1G6/1777274316992.png',
  visionMission: 'https://i.postimg.cc/bwvjXJzh/1777274287552-1.jpg',
  story:         'https://i.postimg.cc/SKB0zfcm/1777274278924-1.jpg',
};

// ── HELPER: send new message with photo + caption + buttons ──
function sendPhoto(chatId, imgUrl, caption, replyMarkup) {
...
// ── HELPER: edit existing message → delete it, send new photo message ──
function editToPhoto(chatId, messageId, imgUrl, caption, replyMarkup) {
  bot.deleteMessage(chatId, messageId).catch(() => {});
  return sendPhoto(chatId, imgUrl, caption, replyMarkup);
}
...
bot.on('message', (msg) => {
  ...
  if (text.toLowerCase() === '/currency') {
    ...
    sendPhoto(chatId, IMG.currency, caption, {
      inline_keyboard: [
...
bot.on('callback_query', (query) => {
  ...
  if (data === 'lang_AM' || data === 'lang_EN') {
    ...
    editToPhoto(chatId, messageId, IMG.mainMenu, caption, getMainMenu(chatId));
    return;
  }
  ...
  if (data === 'fill_form' || data === 'edit_form') {
    ...
    editToPhoto(chatId, messageId, IMG.form, caption, null);
    return;
  }
  ...
  if (data === 'packages') {
    ...
    editToPhoto(chatId, messageId, IMG.packages, caption, {
      inline_keyboard: [
...
<b>የአገልግሎት ጊዜ ይምረጡ:</b>`;
  ...
  // ── Duration / Price ──
  const packageInfo = {
    'dur_lus_1': { price: 20000, name: 'GILD Luster',   months: '1', back: 'pkg_luster'  },
    'dur_lus_2': { price: 35000, name: 'GILD Luster',   months: '2', back: 'pkg_luster'  },
    'dur_lus_3': { price: 50000, name: 'GILD Luster',   months: '3', back: 'pkg_luster'  },
    'dur_rad_1': { price: 35000, name: 'GILD Radiant',  months: '1', back: 'pkg_radiant' },
    'dur_rad_2': { price: 65000, name: 'GILD Radiant',  months: '2', back: 'pkg_radiant' },
    'dur_rad_3': { price: 90000, name: 'GILD Radiant',  months: '3', back: 'pkg_radiant' },
    'dur_24k_1': { price: 70000, name: 'GILD 24K',      months: '1', back: 'pkg_24k'    },
    'dur_24k_2': { price: 130000,name: 'GILD 24K',      months: '2', back: 'pkg_24k'    },
    'dur_24k_3': { price: 205000,name: 'GILD 24K',      months: '3', back: 'pkg_24k'    }
  };
    ...
    bot.editMessageText(successText, {
      ...
      reply_markup: {
        inline_keyboard: [
          ...
          [{ text: isEn ? "🔙 BACK" : "🔙 ወደ ኋላ", callback_data: info.back }]
        ]
      }
    });
    return;
...
/Brand /LandingPage /Website /Bot /Logo /SEO /Strategy /BusinessCard` :
...
/Brand /LandingPage /Website /Bot /Logo /SEO /Strategy /BusinessCard`;
  ...
  if (data === 'more') {
    ...
    editToPhoto(chatId, messageId, IMG.legacy, caption, {
      inline_keyboard: [
  ...
  if (data === 'faq') {
    const caption = isEn
      ...
      : "<b>ግልፅነት እና ጥራት።</b> ❓\n\nጊዜዎን እናከብራለን። በሚሰጧቸው አገልግሎቶች ዙሪያ የተደጋጋሙ ጥያቄዎችን እና ምላሾችን እዚህ ያገኛሉ።";
  ...
  if (data === 'story') {
    const caption = isEn
      ? "<b>The Genesis of Excellence.</b> 📜\n\nThe story of how GILD was forged."
      : "<b>የስኬት መጀመሪያ።</b> 📜\n\nGILD እንዴት እንደተቀረጸ።";
    ...
    const storyAm = "<b>የስኬት መጀመሪያ።</b> 📜\n\n<b>የብራንዶች አልኬሚ (The Alchemy of Brands)</b>\n\n<i>\"እውነተኛ ጥበብ ማለት በተራ ነገሮች ውስጥ የተደበቀውን ወርቅ ማየት መቻል ነው፡፡\"</i>\n\nበጥንታዊው ዘመን፣ <b>አልኬሚ (Alchemy)</b> የሚባል እጅግ ምስጢራዊ ፍልስፍና ነበር፡፡ የጥንት አልኬሚስቶች ትልቁ ምኞት፣ ተራ የሆኑትን ብረቶች ወደ ንፁህ፣ አንፀባራቂ እና ውድ ወርቅነት መቀየር ነበር፡፡\n\n<i>\"ዕንቁ በጭቃ ውስጥ ቢወድቅም እሴቱን አያጣም፤ ነገር ግን እንዲያበራ ጭቃው መገፈፍ አለበት፡፡\"</i>\n\nእኛ በ <b>GILD</b>፣ እራሳችንን እንደ ዘመኑ አልኬሚስቶች እንቆጥራለን፦\n\n<b>1. መገፈፍ (The Peeling):</b> የቆዩ እና የብራንድዎን ውበት የሸፈኑ ንብርብሮችን እንገፍፋለን፡፡\n<b>2. መቅረጽ (The Forging):</b> በስነ-ልቦናዊ ስትራቴጂ ጠንካራ የብራንድ ማንነት እንቀርጻለን፡፡\n<b>3. መለበጥ (The Gilding):</b> ብራንድዎ በገበያው ውስጥ የላቀ ክብር እንዲኖረው እናደርጋለን፡፡\n\n<b>GILD Marketing Agency</b>\n<i>Where Vision Meets Alchemy.</i> ✨";
    editToPhoto(chatId, messageId, IMG.story, caption, {
      inline_keyboard: [[{ text: isEn ? "🔙 BACK" : "🔙 ወደ ኋላ", callback_data: 'more' }]]
    });
    setTimeout(() => {
      bot.sendMessage(chatId, isEn ? storyEn : storyAm, { parse_mode: 'HTML' });
    }, 500);
    return;
  }
  ...
  if (data === 'vision_mission') {
    ...
    editToPhoto(chatId, messageId, IMG.visionMission, caption, {
      inline_keyboard: [
  ...
  if (data === 'our_platform') {
    const caption = isEn
      ...
      : "<b>ዘመናዊ መሠረተ-ልማት።</b> 🌐\n\nየብራንድዎን እድገት ለማፋጠን የምንጠቀምባቸውን የዲጂታል አውታሮች እዚህ ያገኛሉ።";
    editToPhoto(chatId, messageId, IMG.platform, caption, {
      inline_keyboard: [
  ...
  if (data === 'support') {
    const caption = isEn
      ...
      : "<b>ልዩ የድጋፍ አገልግሎት።</b> 🛎️\n\nየእኛ የድጋፍ ቡድን ለማንኛውም አይነት ጥያቄ ዝግጁ ሆኖ ይጠብቅዎታል።";
    editToPhoto(chatId, messageId, IMG.support, caption, {
      inline_keyboard: [
